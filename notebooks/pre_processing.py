...
import glob
from _principal_component_analysis import pca_lazy
...
datetime_prefix = ['pickup', 'dropoff', 'request', 'on_scene']
datetime_col = ['pickup_datetime', 'dropoff_datetime', 'request_datetime', 'on_scene_datetime']
boolean_col = ['shared_request_flag', 'shared_match_flag', 'access_a_ride_flag', 'wav_request_flag', 'wav_match_flag']
...
def derive_data(data, schema = None):
    ...
    data = data.with_columns(wait_time_flag =(pl.col('pickup_time').sub(pl.col('on_scene_time')) > pl.duration(minutes=2)).cast(dtype = pl.Boolean),
                              driver_take_home = pl.sum_horizontal(['driver_pay', 'tips']),
                              final_rider_fare = pl.sum_horizontal(*final_rider_cost_cols),

                                    ).with_columns(wait_time_charged = pl.when(pl.col.wait_time_flag).then(pl.col('pickup_time').sub(pl.col('on_scene_time')).sub(pl.duration(minutes=2))).otherwise(pl.duration()),
                                                   tip_percentage = pl.when(pl.col('tips') == 0).then(0).when(pl.col('final_rider_fare') <= 0).then(pl.col('tips') / pl.sum_horizontal('driver_pay', 'tolls', 'bcf', 'sales_tax', 'cbd_congestion_fee', 'airport_fee')).otherwise(pl.col('tips') / pl.col('final_rider_fare')),
                                                    ...
                                                    ).with_columns(wait_time_fee = pl.when(pl.col.wait_time_flag).then(pl.col('wait_time_charged').dt.total_minutes() * 0.60).otherwise(0),
